...
def query_db():
    if request.method == "POST":
        try:
            q = db_session.execute(request.form["query"])
        except Exception, e:
            flash(e, "error")
            return render_template("admin/querydb.html", query=False)
        if not q.returns_rows:
            db_session.commit()
            q = False
        return render_template("admin/querydb.html", query=q)
    return render_template("admin/querydb.html", query=False)
